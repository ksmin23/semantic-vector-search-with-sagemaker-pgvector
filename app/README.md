...
2. Run the following commands on the terminal to clone the code repository for this post and install the Python packages needed by the application:
   ...
   cd semantic-vector-search-with-sagemaker-pgvector/app
   python -m venv .env
...
3. In the shell, set the following environment variables with the values that are available from the CloudFormation stack output.
   ```
   export POSTGRES_CONNECTION_STRING='postgresql+psycopg2://postgres:{password}@{aurora_postgresql_cluster_endpoint}:5432/'
   export EMBEDDING_ENDPOINT_NAME=hf-sentence-transfomers
   ```
...
![vector-seach-with-pgvector](./vector-seach-with-pgvector.png)
